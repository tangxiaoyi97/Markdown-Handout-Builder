...
The npm package does not vendor these dependencies. When installed through npm, dependency packages are installed separately and include their own license files and package metadata. This notice lists direct dependencies and selected locked PDF-pipeline dependencies; `package-lock.json` is the complete dependency inventory.

Generated handout output may copy KaTeX font files into
`dist/assets/katex-fonts/`. Obsidian-dialect books containing Mermaid diagrams
also copy Mermaid's browser bundle and license to `dist/assets/mermaid.min.js`
and `dist/assets/mermaid.LICENSE`. Those assets remain under their upstream
licenses; Mermaid's minified distribution retains bundled dependency notices.
...
## Selected Locked Transitive Dependencies
